...
yolomedia 在固定类别模型找不到目标时，可用本后端根据文本提示词做开放词汇分割。
返回值统一成 masks/boxes/cls_ids/names/ids，减少上层对 Ultralytics 结果结构的依赖。
"""
...
class YoloEBackend:
    """封装 Ultralytics YOLOE track 接口，输出上层易消费的字典。"""
