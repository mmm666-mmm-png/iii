...
导航提示优先走本模块：根据中文提示文本匹配 voice/map.zh-CN.json 中的 wav，
预加载/可选压缩后放入播放队列，再通过 audio_stream.broadcast_pcm16_realtime
下行到浏览器或设备扬声器链路。
"""
...
def _get_recorder():
    """延迟导入录制器，避免 audio_player 和 sync_recorder 互相导入。"""
...
# 兼容旧工程中的示例音频（保留）
AUDIO_BASE_DIR = os.getenv("AUDIO_BASE_DIR", os.path.join(BASE_DIR, "music"))

# 新增：voice 目录与映射表
# 使用脚本所在目录的 voice 文件夹，避免工作目录问题
VOICE_DIR = os.getenv("VOICE_DIR", os.path.join(BASE_DIR, "voice"))
...
def _merge_voice_map():
    """读取 voice/map.zh-CN.json，把中文提示文本映射到本地 wav 文件。"""
    try:
...
def _audio_worker():
    """独立播放线程：从优先级队列取 PCM，切回 asyncio loop 广播。"""
    global _worker_loop
...
def play_voice_text(text: str):
    ...
    # 路线规划播报是动态文本（含距离/时间/盲道覆盖率），无法命中静态 wav，
    # 降级到通用的“已为您规划好路线”播报，避免每次规划都报“未找到匹配语音”。
    if t.startswith("已为您规划好路线") or "已为您规划好路线" in t:
        fallback = "已为您规划好路线，请确认安全后出发。"
        if fallback in AUDIO_MAP:
            play_audio_threadsafe(fallback)
            _last_voice_text = text
            _last_voice_time = current_time
