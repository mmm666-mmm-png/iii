...
当前 app_main.py 主要使用 process_single_frame 的“单帧模式”，避免单独线程抢帧；
main/start_detection 保留给独立调试或旧版工作流使用。
"""
...
YOLO_MODEL_PATH = os.path.join(BASE_DIR, "..", "AIGlasses_for_navigation", "trafficlight.pt")
...
def main(headless: bool = True, stop_event=None):
    """
    红绿灯检测主函数。独立线程/脚本模式使用，从 bridge_io 拉取最新帧并推回预览。
...
def process_single_frame(image: np.ndarray, ui_broadcast_callback=None) -> dict:
    """
    处理单帧图像（主线程模式，避免掉帧）。
    只返回检测状态和标注图，不直接播报语音，语音由上层状态机统一控制。
    参数：
        image: 输入图像
    ...
    # YOLO 推理。模型类别里包含 crossing/blank 等非灯类别，后面会过滤。
    ...
    # 状态稳定性判断：最近 5 帧中至少 2 帧同类，即认为稳定。
    # 这个阈值偏灵敏，适合低帧率实时导航；误报需要在模型或场地侧继续调。
