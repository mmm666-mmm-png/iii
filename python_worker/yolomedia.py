...
在本项目中它作为“找物品/抓取引导”后台线程运行：
app_main.py 通过 bridge_io 推入摄像头帧，yolomedia 识别目标和手部位置，
再把标注画面和方向语音提示推回前端/音频系统。
"""
...
YOLO_MODEL_PATH = os.path.join(BASE_DIR, "..", "AIGlasses_for_navigation", "shoppingbest5.pt")
...
# 初始化 pygame 音频。云服务器通常没有 ALSA 声卡，初始化失败不能影响 worker 启动。
try:
    pygame.mixer.init()
except Exception as e:
    print(f"[AUDIO] pygame mixer disabled: {e}", flush=True)
...
# ======== TTS（pyttsx3）========
class Speaker:
    """本地 pyttsx3 语音播报封装；多数部署会改用 audio_player 的预录语音。"""
...
def hand_bbox_and_area(lms, W, H):
    """根据 MediaPipe 手部关键点估计手的 bbox 和面积。"""
...
def check_hand_object_contact(hand_box, poly, overlap_threshold=0.15):
    """检查手部 bbox 与目标多边形是否有足够重叠，辅助判断接触/抓取。"""
    """
    检测手的边界框和物体多边形是否有重叠
    返回: (是否接触, 重叠比例)
    """
...
def get_guidance_direction(hand_center, object_center, hand_area, object_area, hand_box=None, poly=None):
    """根据手和目标的位置/面积关系生成“向左/向右/向前/后退/OK”等引导。"""
    """
...
def main(headless: bool = False, prompt_name: str = None, stop_event=None):
    ...
    prompt_name 是目标类别名；stop_event 由 app_main.py 控制退出。
    headless=True 时不打开 OpenCV 窗口，只通过 bridge_io.send_vis_bgr 输出画面。
    """
