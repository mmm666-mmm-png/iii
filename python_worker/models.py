...
def model_path(filename: str) -> str:
    return os.path.join(BASE_DIR, "..", "AIGlasses_for_navigation", filename)
...
def init_all_models():
    ...
    try:
        ...
        crosswalk_detector_client = CrosswalkDetector(model_path=model_path("yolo-seg.pt"))
        # 将其内部的YOLO模型移动到指定设备
        ...
        coco_client = COCOClient(model_path=model_path("yolov8l-world.pt"))
        # 将其内部的YOLO模型移动到指定设备
        ...
        blindpath_seg_model = YOLO(model_path("yolo-seg.pt"))
