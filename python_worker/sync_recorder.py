...
默认关闭，设置 ENABLE_SYNC_RECORDER=1 后启用。录制器按固定 FPS 写视频，
并用静音填充音频空档，让视频和语音提示在时间轴上大致对齐，便于调试导航行为。
"""
...
RECORDER_ENV = "ENABLE_SYNC_RECORDER"

def _recorder_enabled():
    """读取环境变量开关，避免默认运行时持续写磁盘。"""
    return os.getenv(RECORDER_ENV, "").strip().lower() in ("1", "true", "yes", "on")

class SyncRecorder:
    """同步录制器：负责一个录制会话中的 AVI 视频和 WAV 音频。"""
    ...
    def __init__(self, output_dir="recordings", fps=15.0):
        ...
        # 音频参数：这里保存为 16kHz 16bit mono，便于后期常见播放器/工具分析。
    ...
    def start_recording(self):
        """开始新的录制会话，按当前时间戳创建 video_*.avi 和 audio_*.wav。"""
    ...
    def add_frame(self, jpeg_data: bytes):
        ...
        try:
            with self.lock:
                ...
                # 每写一帧就把音频补齐到对应视频时长，防止长时间无语音时音轨短于视频。
...
def start_recording():
    ...
    if not _recorder_enabled():
        print(f"[RECORDER] 同步录制默认关闭；设置 {RECORDER_ENV}=1 可开启")
        return False
