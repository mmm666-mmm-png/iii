...
它被 BlindPathNavigator 调用，用于在仍处于盲道导航阶段时提前提示用户：
远处发现斑马线、正在靠近、接近、到达等。真正切换过马路状态由 NavigationMaster 决定。
"""
import time
import numpy as np
...
class CrosswalkAwarenessMonitor:
    ...
    def process_frame(self, crosswalk_mask, blind_path_mask=None) -> Optional[Dict[str, Any]]:
        ...
        # 4. 检查遮挡。盲道与斑马线重叠过多时，避免过早提示“可以过马路”。
