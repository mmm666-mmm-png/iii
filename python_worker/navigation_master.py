...
单个盲道或过马路工作流只关心自己的视觉任务；NavigationMaster 负责把它们串起来：
- CHAT/IDLE：普通问答，不做导航推理；
- BLINDPATH_NAV：沿盲道行走；
- SEEKING_CROSSWALK/WAIT_TRAFFIC_LIGHT/CROSSING：发现斑马线、等灯、过马路；
- SEEKING_NEXT_BLINDPATH：过完马路后寻找下一段盲道；
- ITEM_SEARCH：暂停导航，交给 yolomedia 找物品。
"""
import time
import math
import cv2
...
class OrchestratorResult:
    """对 app_main.py 的统一返回：标注图、语音提示、状态和调试字段。"""
...
class NavigationMaster:
    ...
    它不直接播放音频，只返回 guidance_text；上层 app_main.py 负责节流后播放预录语音。
    """
    def __init__(self,
    ...
    def force_state(self, s: str):
        """强制切换状态，主要用于 Omni 问答临时进入 CHAT 后恢复原状态。"""
        self.state = s
        self.cooldown_until = time.time() + self.COOLDOWN_SEC

    def on_voice_command(self, text: str):
        """处理少量跨状态语音命令，例如“立即通过”“继续”“停止”。"""
        t = (text or "").strip()
        if "开始过马路" in t:
    ...
    def reset(self):
        """重置统领器和子工作流，通常在摄像头断开或外部 reset 命令时调用。"""
        self.state = IDLE
    ...
    def process_frame(self, bgr: np.ndarray) -> OrchestratorResult:
        ...
        根据当前状态选择盲道、红绿灯或过马路工作流，并根据稳定计数决定状态迁移。
        返回值中的 guidance_text 可能为空，表示当前帧不需要播报。
        """
        now = time.time()
        ...
        if self.state in (BLINDPATH_NAV, SEEKING_CROSSWALK, SEEKING_NEXT_BLINDPATH, RECOVERY):
            # 盲道相关状态都先调用 BlindPathNavigator，再根据其 state_info 判断是否转入过马路阶段。
            # —— 盲道侧 —— 统一调用盲道导航器
            try:
            ...
            if self.state == BLINDPATH_NAV:
                # 盲道阶段连续多帧看到斑马线，才切换到 SEEKING_CROSSWALK，降低误检抖动。
            ...
            # —— 对准阶段：同时利用 blind 内部 crosswalk_tracker 的角度与偏移（若提供）
                ...
                # 到达斑马线前要求“ready + 角度/偏移对准”连续稳定一段时间。
            ...
            # —— 过马路后寻找下一段盲道（上盲道流程）
                ...
                # 过完马路后，如果盲道工作流重新稳定进入 NAVIGATING，就回到盲道导航。
            ...
            if blind_state == "UNKNOWN" and cross_stage == "not_detected":
                # 兜底丢失计数：连续找不到盲道/斑马线时进入 RECOVERY。
                self.cnt_lost += 1
            else:
        ...
        if self.state == WAIT_TRAFFIC_LIGHT:
            # 等灯阶段用颜色检测 + 多数表决；稳定绿灯后自动进入 CROSSING。
            ann = bgr.copy()
            # 红绿灯识别（多数表决+冷却）
        ...
        if self.state == CROSSING:
            # 过马路阶段交给 CrossStreetNavigator；当它检测到下一段盲道足够近时切回盲道导航。
            try:
