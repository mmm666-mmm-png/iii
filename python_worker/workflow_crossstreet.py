...
它只负责过马路阶段的局部判断；是否从盲道切入/切出由 NavigationMaster 管理。
"""
import torch
import os
import time
...
class CrossStreetNavigator:
    ...
    主要根据斑马线掩码计算角度、横向偏移、远近程度，并在 CROSSING 阶段寻找下一段盲道。
    ...
    def process_frame(self, bgr_image: np.ndarray) -> CrossStreetResult:
        ...
        输出 guidance_text 给上层播报；如果检测到下一段盲道并满足切换条件，
        会设置 should_switch_to_blindpath，由 NavigationMaster 完成状态迁移。
        """
