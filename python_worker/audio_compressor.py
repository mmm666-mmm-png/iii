...
当前主要被 audio_player.py 的预录语音缓存使用：
启动时把 wav 转成 8kHz PCM，再按 ulaw/adpcm/none 缓存在内存里；
真正播放前再解压回 PCM16，交给 audio_stream 按 20ms 节拍下行。
"""
import os
import wave
import struct
...
class AudioCompressor:
    ...
    ulaw 算法简单、压缩率 50%；ADPCM 压缩率更高，语音提示质量也够用。
    这里只处理 mono PCM16，不做复杂容器封装。
    """
    ...
    def pcm16_to_ulaw(pcm_data: bytes) -> bytes:
        ...
        压缩率：50%（16bit -> 8bit）
        """
        # 解析16位PCM。np.frombuffer 不复制数据，适合启动阶段批量转换。
    ...
    def _linear_to_ulaw(sample: int) -> int:
        ...
        16位线性PCM转μ-law
        """
        # μ-law 常量。BIAS 用于压缩小信号，MAX 避免指数计算越界。
        ULAW_MAX = 0x1FFF
        ULAW_BIAS = 0x84
    ...
    def pcm16_to_adpcm(pcm_data: bytes) -> bytes:
        ...
        # ADPCM 是预测编码：保存上一预测值和步长索引，每个样本只写 4 bit 差分。
        ...
        # 添加头部信息：最终预测值和步长索引。解码端需要从同一状态恢复。
...
class CompressedAudioCache:
    """压缩音频缓存：以文件路径为 key，避免同一 wav 被重复读取/转换。"""
    ...
    def load_and_compress(self, filepath: str) -> Optional[bytes]:
        """加载并压缩音频文件，统一转换为 8kHz mono PCM 后再编码。"""
        ...
        try:
            with wave.open(filepath, 'rb') as wav:
                ...
                # 压缩数据前加 5 字节自定义头：
                # 1 字节算法标识 + 4 字节原始 PCM 长度，便于播放前识别和统计。
