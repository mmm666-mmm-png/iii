...
# -*- coding: utf-8 -*-
"""
DashScope 实时 ASR 回调封装。

本模块不直接创建 WebSocket 或 FastAPI 路由，只负责：
1. 兼容 DashScope SDK 不同版本的事件对象结构；
2. 从事件中抽取 partial/final 文本；
...
def _safe_to_dict(x: Any) -> Dict[str, Any]:
    ...
    DashScope SDK 的回调对象在不同版本中可能是 dict、带 to_dict/model_dump 的对象，
    或仅能转成字符串；这里做宽容处理，避免某个版本差异导致 ASR 全链路失败。
    """
