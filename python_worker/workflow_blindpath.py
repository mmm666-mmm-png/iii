...
核心输入是一帧 BGR 图像，输出 ProcessingResult：
- guidance_text：本帧建议播报的中文提示；
- annotated_image：叠加盲道/障碍物/斑马线等可视化后的画面；
- state_info：给 NavigationMaster 判断跨流程切换的结构化状态。
"""
import os
import time
import threading
...
class BlindPathNavigator:
    ...
    内部包含 ONBOARDING、NAVIGATING、TURN、AVOIDING_OBSTACLE 等子状态，
    并在每帧中同时检测盲道、斑马线和障碍物。
    """
    ...
    def process_frame(self, image: np.ndarray) -> ProcessingResult:
        ...
        # 1. 盲道/斑马线分割。这里直接用当前帧结果，避免旧掩码造成错误引导。
        ...
        # 2. 全程障碍物检测。无论处于校准、直行、转弯还是避障状态，都要优先发现危险物。
