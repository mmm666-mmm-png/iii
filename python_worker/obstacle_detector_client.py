...
盲道和过马路工作流都可以复用本类。它把一组白名单英文类别作为文本提示词，
预计算 text embedding 后在每次 detect 时进行分割检测，并可按 path_mask 过滤路径外物体。
"""
import logging
import os
import cv2
...
@contextmanager
def gpu_infer_slot():
    """统一管理 GPU 并发限流、torch.inference_mode 和 AMP autocast。"""
    with _gpu_slots:
    ...
    def detect(self, image: np.ndarray, path_mask: np.ndarray = None) -> List[Dict[str, Any]]:
        ...
        for i, mask_tensor in enumerate(results[0].masks.data):
            ...
            if path_mask is not None:
                # path_mask 代表盲道/通行区域，只保留与路径有交集的障碍物。
